...
def get_ckpt_model(path='', load_language_model=False, load_image_encoder=True, path2=''):
    ...
    if load_language_model:
        checkpoint = torch.load('path_to_trained_language_model', map_location='cpu')
        ckpt_pth = checkpoint['model_state_dict']
...
def blip2qformerlayer_selfforward(layer, hidden_states,attention_mask=None,head_mask=None,encoder_hidden_states=None,encoder_attention_mask=None,
    ...
    if query_length > 0:
        ...
        if attention_output.shape[1] > query_length:
            ...
            # layer_output_text = attention_output[:, query_length:, :]
            layer_output = torch.cat([layer_output[:, :32, :], layer_output_text[:, 32:, :]], dim=1)
    else:
        layer_output = apply_chunking_to_forward(
...
def aokvqa_generate_full_rationale(path='', load_image_encoder=True, path2='', add_rationale=False, use_boundingbox=False, step=3, path_name=''):
    tokenizer = AutoTokenizer.from_pretrained(VERSION)
    processor, model = get_ckpt_model(path, load_image_encoder=load_image_encoder, path2=path2)
    eval_dataset = AOKVQAEvalDataset(processor=processor, data_path="data",use_boundingbox=use_boundingbox)
    batch_size = 2
    ...
    if path != '':
        ...
        path_ = '_pretrained3'
    with open('path_to_previous_step_generated_rational', 'r') as f:
    ...
    for index_tuple in indexes:
        ...
        for num,i in enumerate(index_tuple):
            if output_text[num] in answer_list[num]:
                correct += 1
...
def vqax_generate_full_rationale(path='', load_image_encoder=True, path2='', add_rationale=False, use_boundingbox=False, step=3, path_name = '', self_only=True):
    tokenizer = AutoTokenizer.from_pretrained(VERSION)
    processor, model = get_ckpt_model(path, load_image_encoder=load_image_encoder, path2=path2)
    eval_dataset = VQAXEvalDataset(processor=processor, data_path="data",use_boundingbox=use_boundingbox)
    batch_size = 4
    progress_bar = tqdm(range(int(len(eval_dataset)/batch_size)))
    indexes_list = list(range(0, len(eval_dataset)))
    indexes = [tuple([indexes_list[i+j] for j in range(0, min(batch_size, len(eval_dataset)-0-i))]) for i in range(0, len(eval_dataset)-0, batch_size)]
...
if __name__=='__main__':
    ...
    path_qformer = 'path_to_trained_qformer'
    path = 'path_to_trained_image_encoder'
    vqax_generate_full_rationale(path=path_qformer, load_image_encoder=True, path2=path, add_rationale=True, use_boundingbox=False, path_name='vqax_selftrain_32shot')

     
    """okvqa"""
    path_qformer = 'path_to_trained_qformer'
    path = 'path_to_trained_image_encoder'
    aokvqa_generate_full_rationale(path=path_qformer, load_image_encoder=True, path2=path, add_rationale=True, use_boundingbox=False, path_name='aokvqa_selftrainbefore_lengthpenalty')
